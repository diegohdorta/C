...
void *handle_client(void *arg){
	...
	sprintf(buff_out, "<< Hello, bitches! Welcome to our chat! >> %s\r\n", cli->name);
		...
		if(buff_in[0] == '\\'){
			...
			if(!strcmp(command, "\\QUIT")){
				...
				if(param){
					...
					sprintf(buff_out, "<<RENAME POKEMON, %s TO %s\r\n", old_name, cli->name);
					free(old_name);
					send_message_all(buff_out);
				}else{
					send_message_self("<<NAME CANNOT BE NULL, YOUR IDIOT!\r\n", cli->connfd);
				}
			...
			if (cli->uid == 10)
				...
				sprintf(buff_out, BLUE "[%s] %s\r\n" NORMAL, cli->name, buff_in);

			else if (cli->uid == 11)

				sprintf(buff_out, RED "[%s] %s\r\n" NORMAL, cli->name, buff_in);

			else if (cli->uid == 12)

				sprintf(buff_out, GREEN "[%s] %s\r\n" NORMAL, cli->name, buff_in);

			else if (cli->uid == 13)

				sprintf(buff_out, YELLOW "[%s] %s\r\n" NORMAL, cli->name, buff_in);

			else if (cli->uid == 14)

				sprintf(buff_out, CYAN "[%s] %s\r\n" NORMAL, cli->name, buff_in);

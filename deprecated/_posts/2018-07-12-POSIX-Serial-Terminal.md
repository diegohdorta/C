---
layout: post
title: "Creating POSIX Serial Terminal"
author: "Diego Dorta"
categories: unix
tags: [ posix, serial, terminal, c, language, imx ]
---

The following code was written intending to provide a very simple tool to use it as a `serial terminal` for `iMX`, it works similar as a `minicom`.
The [code](https://github.com/diegohdorta/feynman/blob/master/serial/serial.c) was developed using *POSIX* and *C* language.
To compile the following code just use regular instruction:

```bash
$ gcc code.c -o serial
```
Then, just run the executable file passing the `device` as an argument:

```bash
$ ./serial /dev/ttyUSB0
```
If not specified, the default will be: `/dev/ttyS0`. 

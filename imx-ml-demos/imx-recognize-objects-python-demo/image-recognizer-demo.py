...
ARMNN_RUN = "TfMobileNet-Armnn --data-dir=data --model-dir=models > log.txt"
PARSER_SCRIPT = "./parser.sh"
INPUT_PATH = "/run/media/mmcblk1p3/ArmnnTests/data/"
UGLY_HACK = "Dog.jpg"
RECTANGULE = 4
READY = 8
...
def create_mask_crop_rectangule_content(f, r):
    m = f.copy()
...
def main(count=0):
    ...
    while True:
        ...
        if (len(result) == RECTANGULE):
            draw_contours(frame, result)
            count += 1
        else: count = 0

        if (count == READY):
            mask = create_mask_crop_rectangule_content(frame, result)            

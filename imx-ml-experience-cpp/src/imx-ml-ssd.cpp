...
{  
    ...
    this->inputBlob = blobFromImage(this->new_img, 0.007843, Size(300, 300), Scalar(127.5, 127.5, 127.5), false, false);
    ...
    for (int i = 0; i < detectionMat.rows; i++) {
        ...
        if (confidence > 0.7) {
            ...
            std::string label = format("%s: %.4f", (this->classes.empty() ? format("Class #%d", classId).c_str() :  this->classes[classId].c_str()), confidence);

            Size labelSize = getTextSize(label, FONT_HERSHEY_TRIPLEX, 0.5, 1, &baseLine);
            rectangle(this->img, Rect(Point(xLeftBottom, yLeftBottom - labelSize.height), Size(labelSize.width, labelSize.height + baseLine)), Scalar(190, 200, 255), 0);
            putText(this->img, label, Point(xLeftBottom, yLeftBottom), FONT_HERSHEY_DUPLEX, 1, Scalar(255,255,255));
        }
    }
    imwrite("test2.jpg", this->img);  
    auto end = std::chrono::system_clock::now();
    elapsed_seconds = end-start;

    Glib::RefPtr<Gdk::Pixbuf> refPixbuf = Gdk::Pixbuf::create_from_file("test2.jpg", 640, 480, true);
    this->ImagePtr->set(refPixbuf);

    //elapsed_seconds = elapsed_seconds.count();
    // usar glib para criar pastas em /tmp e salver as imagens la fornecendo o link depois da execucao, assim como o tempo de execucao 
    return true;    
}  

...
RobinNN::RobinNN(std::vector<int> neurons, double learningRate)
{
    srand (time(NULL));
...
RobinNN::RobinNN(const char *filepath)
{
    loadRobinNNParams(filepath);
}

Matrix<double> RobinNN::computeOutput(std::vector<double> input)
{
...
void RobinNN::learn(std::vector<double> expectedOutput)
{
...
void RobinNN::printToFile(Matrix<double> &m, std::ofstream &file)
{
    int h = m.getHeight();
    int w = m.getWidth();
...
void RobinNN::saveRobinNNParams(const char *filepath)
{
...
void RobinNN::loadRobinNNParams(const char *filepath)
{
...
double RobinNN::random(double x)
{
    return (double)(rand() % 10000 + 1)/10000-0.5;
}

double RobinNN::sigmoid(double x)
{
    return 1/(1+exp(-x));
}

double RobinNN::sigmoidePrime(double x)
{

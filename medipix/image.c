...
void receive_bytes_from_medipix(int s_udp, struct sockaddr_in medipix, struct data *fl, int counter)
{
	...
	buffer buffer_one_bit[64];	// Declarando 64 vetores de 257 posições cada. ?!?!?
	...
	fprintf(stderr, "Waiting for medipix sending the bytes image...\n");
	...
	fprintf(stderr, "Treatment of the received data...\n");
	...
	for (i = 0; i <= 255; i++) {
    	...
    	fprintf(stderr, "File created with SUCCESS...\n\n");

...
int accept_new_connection(int s)
{
	...
	fprintf(stderr, "The client IOC IP %s is now connected!\n", inet_ntoa(ioc.sin_addr));
	return ns;
}
...
void receive_variables(struct data *fl)
{
	...
	do {
		fprintf(stderr, "Waiting for variables...\n");
		done = false;
		i = 0;
		do {
			...
			if (size == 0) {
				fprintf(stderr, "The IOC was turned off!\n");
				fl->ret = false;
				return;				
			}
			...
			for (x = 0; x < size; x++) {
				...
				fl->variable[i] = buffer[x];
							
				i++;
				if (i == variable_size) {
					fprintf(fl->log_error, "Buffer full...\n");
		...
		switch(fl->variable[0]) {

			case ID_FILENAME:
				strcpy(fl->filename,(&(fl->variable[1])));
				fprintf(stderr, "Filename: %s\n",fl->filename);			
				break;
			case ID_FRAMES:
				fl->number_frames = atoi(&(fl->variable[1]));
				fprintf(stderr, "Frames: %d\n",fl->number_frames);			
				break;
			case ID_BITS:
				fl->number_bits = atoi(&(fl->variable[1]));
				fprintf(stderr, "Bits: %d\n",fl->number_bits);			
				break;
			default:
				fprintf(fl->log_error, "Invalid variables types\n");
				fprintf(stderr, "Sending message error to IOC\n");
		...
		fprintf(stderr, "Content of buffer: %s\n", fl->variable);

...
void *thread_shandler(void *data)
{
	thread_t *info = (thread_t *)data;
	
	shandler(info->s, info->client, info->g_shm);
	
	return NULL;
}

void shandler(int *ns, struct sockaddr_in client, shm_t *shm_storage)
{
	int i, rtn;
